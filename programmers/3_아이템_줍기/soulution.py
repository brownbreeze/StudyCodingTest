...
def solution(rectangle, characterX, characterY, itemX, itemY):
    ...
    arr = deque()      
    for i in range(4):
        zeroy = characterY*2 + go_pos[0][i]
        zerox = characterX*2 + go_pos[1][i]
        if maps[zeroy][zerox] == 0:
            arr.append([0, characterY*2, characterX*2, (i+1)%4]) 
            break
    result = bfs(maps, arr, itemY*2, itemX*2)
    result = total_len - result if total_len - result < result else result
    return result//2
